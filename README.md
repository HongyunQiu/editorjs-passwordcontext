...
说明：点击“隐藏”或保存时，使用用户输入的密码派生密钥（PBKDF2，SHA-256，100000 次，含随机 salt）对内容进行 AES-GCM 加密。再次打开时需输入正确密码解锁；若密码错误将无法解密。

---

安全与密码设定建议（简述）：

- 纯前端明文对比仅用于演示，不提供真实安全性；用户可在前端源码或内存中获取内容。
- 如需“真正”保护：
  - 方案 A：后端鉴权 + 受保护内容仅后端下发，前端不保存明文。
  - 方案 B：前端加密存储（AES-GCM）+ 密码派生（PBKDF2/Argon2）+ 随机盐与 IV；仅在输入密码后解密。
  - 密码永不明文存储；在后端保存哈希/派生值与密文，前端只拿密文，解锁时派生密钥解密。
